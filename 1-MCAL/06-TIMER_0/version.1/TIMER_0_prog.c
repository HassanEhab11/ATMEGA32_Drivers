...
void TIMER0_voidSetTime_ms(u32 Copy_u32DesiredTimeOrDutyCycle)
{
	...
	if (FAST_PWM_MODE == FAST_PWM_INVERTED_MODE)
	{
		OCR0 = (((100 - Copy_u32DesiredTimeOrDutyCycle) * TIMER0_MAX ) / 100) -  1 ;
	}
	else if (FAST_PWM_MODE == FAST_PWM_NON_INVERTED_MODE)
	{
		OCR0 = ((Copy_u32DesiredTimeOrDutyCycle * TIMER0_MAX ) / 100) -1;
	}

#elif TIMER0_MODE == TIMER0_PHASE_CORRECT_PWM
	if (PHASE_CORRECT_PWM_MODE == PHASE_CORRECT_PWM_INVERTED_MODE)
	{
		OCR0 = ((( 100 -Copy_u32DesiredTimeOrDutyCycle ) * (TIMER0_MAX -1 ) ) / 100) ;
	}
	else if (PHASE_CORRECT_PWM_MODE == PHASE_CORRECT_PWM_NON_INVERTED_MODE)
	{
		OCR0 = ((Copy_u32DesiredTimeOrDutyCycle * (TIMER0_MAX -1 ) ) / 100) ;
	}
